...
/*   Updated: 2019/12/01 02:25:47 by mbrignol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_arg_type(va_list va, t_flag *info)
{
	...
	if (info->letter == 'd' || info->letter == 'i' || info->letter == 'x' || info->letter == 'X' || info->letter == 'u' || info->letter == 'p')
...
int		get_int_arg(va_list va, t_flag *info)
{
	...
	if (info->letter == 'd' || info->letter == 'i')
		...
		nbr = ft_itoa_unsigned(va_arg(va, unsigned int));
		 ...
		 nbr = ft_itoa_base(va_arg(va, int), 16, info);
	info->str = nbr;
	//free(nbr);
	return (1);
}
